...
class TestRawSnapperResolutionFixes:
    """Duplicate snapper numbers list in a stable order; ssh options are threaded."""
    ...
    def test_endpoint_options_threaded_into_choose_endpoint(self, tmp_path):
        """ssh_sudo/ssh_key flow into choose_endpoint's common_config, for the
        listing and for the source the restore sends from alike."""
        from btrfs_backup_ng.core.restore import (
        ...
        fake.prepare = lambda: None
        with patch("btrfs_backup_ng.endpoint.choose_endpoint", return_value=fake) as ce:
            _snapper_source("raw+ssh://h/p", [], {"ssh_sudo": True})
