...
class LocalEndpoint(Endpoint):
    ...
    def __init__(self, config=None, **kwargs) -> None:
        ...
        if self.config["source"]:
            logger.debug("Original source path: %s (type: %s)", 
        ...
        logger.debug("Original destination path: %s (type: %s)", 
                     self.config["path"], type(self.config["path"]))
        try:
    ...
    def _prepare(self) -> None:
        ...
        try:
            ...
            if not btrfs_path:
                ...
                raise __util__.AbortError("btrfs command not found in system PATH. Please ensure btrfs-progs is installed.")
        ...
        if self.config.get("snapshot_dir") and isinstance(self.config["snapshot_dir"], (str, Path)):
            ...
            try:
                ...
                logger.error("Error creating snapshot directory %s: %s", snapshot_dir, e)
                raise __util__.AbortError(f"Failed to create snapshot directory {snapshot_dir}: {e}")
        ...
        if (
            ...
            raise __util__.AbortError(f"Source {self.config['source']} is not a btrfs subvolume. Use --no-fs-checks to override.")
        ...
        if self.config["fs_checks"] and not __util__.is_btrfs(self.config["path"]):
            ...
            raise __util__.AbortError(f"Destination {self.config['path']} is not on a btrfs filesystem. Use --no-fs-checks to override.")
