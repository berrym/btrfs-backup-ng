"""Type stubs for __util__ module."""

import time
from pathlib import Path
from typing import Any, List, Optional

DATE_FORMAT: str
MOUNTS_FILE: str

class AbortError(Exception): ...
class SnapshotTransferError(AbortError): ...
class InsufficientSpaceError(AbortError): ...

class Snapshot:
    location: Path
    prefix: str
    endpoint: Any
    time_obj: time.struct_time
    time_format: str
    locks: set
    parent_locks: set
    uuid: str
    received_uuid: str

    def __init__(
        self,
        location: str | Path,
        prefix: str,
        endpoint: Any,
        time_obj: Optional[time.struct_time] = None,
        time_format: Optional[str] = None,
    ) -> None: ...
    def __eq__(self, other: object) -> bool: ...
    def __lt__(self, other: "Snapshot") -> bool: ...
    def __repr__(self) -> str: ...
    def get_name(self) -> str: ...
    def get_path(self) -> Path: ...
    def find_parent(
        self, present_snapshots: List["Snapshot"]
    ) -> Optional["Snapshot"]: ...

def exec_subprocess(
    command: List[str], method: str = "check_output", **kwargs: Any
) -> Any: ...
def log_heading(caption: str) -> str: ...
def date_to_str(
    timestamp: Optional[time.struct_time] = None, fmt: Optional[str] = None
) -> str: ...
def str_to_date(
    time_string: Optional[str] = None, fmt: Optional[str] = None
) -> time.struct_time: ...
def parse_snapshot_time(
    time_string: str, preferred_fmt: Optional[str] = None
) -> tuple[time.struct_time, str]: ...
def parse_subvolume_list(output: str) -> list[dict[str, str]]: ...
def parse_subvolume_show(output: str) -> dict[str, str]: ...
def is_btrfs(path: str | Path) -> bool: ...
def is_subvolume(path: str | Path) -> bool: ...
def infer_snapshot_prefix(
    name: str, preferred_fmt: Optional[str] = None
) -> Optional[str]: ...
def read_locks(s: str) -> dict[str, Any]: ...
def write_locks(lock_dict: dict[str, Any]) -> str: ...
def delete_subvolume(path: str | Path) -> None: ...
def is_mounted(path: str | Path) -> bool: ...
def atomic_write_bytes(
    path: str | Path, data: bytes | str, *, mode: int = ..., fsync: bool = ...
) -> None: ...
def privileged_mkdir(
    path: str | Path,
    *,
    parents: bool = ...,
    exist_ok: bool = ...,
    allow_prompt: bool = ...,
) -> None: ...
def privileged_write_bytes(
    path: str | Path, data: bytes | str, *, allow_prompt: bool = ...
) -> None: ...
def privileged_chmod(
    path: str | Path, mode: int, *, allow_prompt: bool = ...
) -> None: ...
def privileged_rmtree(path: str | Path, *, allow_prompt: bool = ...) -> None: ...
