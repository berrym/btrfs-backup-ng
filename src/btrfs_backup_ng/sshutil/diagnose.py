...
def ssh_command_base(host: str, port: Optional[int] = None, identity_file: Optional[str] = None) -> List[str]:
...
def test_ssh_connection(host: str, port: Optional[int] = None, identity_file: Optional[str] = None) -> bool:
...
def test_sudo_access(host: str, port: Optional[int] = None, identity_file: Optional[str] = None) -> bool:
...
def test_btrfs_command(host: str, port: Optional[int] = None, identity_file: Optional[str] = None) -> bool:
...
def test_sudo_btrfs(host: str, port: Optional[int] = None, identity_file: Optional[str] = None) -> bool:
...
def test_write_permissions(host: str, path: str, port: Optional[int] = None, identity_file: Optional[str] = None) -> bool:
    ...
    if returncode != 0:
        ...
        if returncode == 0:
            ...
            logger.error(f"Parent directory {parent_path} does not exist or is not writable")
            return False
...
def test_btrfs_filesystem(host: str, path: str, port: Optional[int] = None, identity_file: Optional[str] = None) -> bool:
    """Test if the path is on a btrfs filesystem."""
    logger.info(f"Testing if {path} is on a btrfs filesystem...")
    
    cmd = ssh_command_base(host, port, identity_file)
    cmd.append(f"stat -f -c '%T' '{path}' 2>/dev/null || stat -f -c '%T' '{str(Path(path).parent)}' 2>/dev/null")
...
def test_btrfs_receive(host: str, path: str, port: Optional[int] = None, identity_file: Optional[str] = None) -> bool:
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        cmd.append(f"sudo -n btrfs subvolume create '{test_path}' && sudo -n btrfs subvolume delete '{test_path}'")
        
        returncode, stdout, stderr = run_command(cmd, timeout=60)
        
        if returncode == 0:
            logger.info("Successfully tested btrfs subvolume create/delete on remote host")
            return True
        else:
...
def create_sudoers_fix_instructions(host: str) -> None:
    ...
    logger.info("=" * 70)
    logger.info("")
    logger.info("To enable passwordless sudo for btrfs commands, run these commands on the remote host:")
    logger.info("")
    logger.info("1. Connect to the remote host:")
    logger.info(f"   ssh {host}")
    logger.info("")
    logger.info("2. Create a sudoers file for btrfs commands:")
    logger.info("   sudo sh -c 'echo \"# Allow passwordless sudo for btrfs commands\" > /etc/sudoers.d/btrfs-backup'")
    logger.info("   sudo sh -c 'echo \"%sudo ALL=(ALL) NOPASSWD: /usr/bin/btrfs\" >> /etc/sudoers.d/btrfs-backup'")
    logger.info("")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Diagnose SSH and sudo issues for btrfs-backup-ng")
    ...
    write_ok = test_write_permissions(args.host, args.path, args.port, args.identity_file)
    btrfs_fs_ok = test_btrfs_filesystem(args.host, args.path, args.port, args.identity_file)
    ...
    if all([ssh_ok, btrfs_ok, sudo_btrfs_ok, write_ok, btrfs_fs_ok]):
        ...
        if not write_ok:
            ...
            logger.info("or that sudo is configured properly to allow writing to this location.")
